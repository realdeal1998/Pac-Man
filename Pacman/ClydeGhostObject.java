...
public class ClydeGhostObject extends GameObject
{
  BufferedImage clydeImage = null , powerUpImage = null , powerUpEndingImage = null;
  
  public ClydeGhostObject(int x , int y , int spawnWait)
  {
    try
    {
      clydeImage = ImageIO.read(new File("clydeSpriteSheet.png"));
  ...
  public void hasToMakeDecision(int[]dataBuffInt , int w , int pacmanXPos , int pacmanYPos  , Ellipse2D.Double pacmanBounds)
  {
    ...
    if(flag ==0)
    {
      ...
      else if(scatterTimer < 2900) makeRandomDecision(dataBuffInt,w);
      else
      {
        if(pacmanXPos==xPos){
          startVertical = true;
  ...
  public void setSpawnSpeeds(int[]dataBuffInt , int w , int pacmanXPos , int pacmanYPos , Ellipse2D.Double pacmanBounds)
  {
    ...
    else if(xPos==531 && yPos==303 && spawnWaitingTime==0)
    {
      left();
    }
  ...
  {
    if(ghostTimer==4) g.drawString(""+eatingScore,xPos,yPos+25);
    else if(!powerUp)g.drawImage(clydeImage,xPos, yPos, xPos+WIDTH, yPos+WIDTH ,frameX,frameY,frameX+WIDTH , frameY+WIDTH,observer);
  ...
  {
     g.drawImage(clydeImage,xPos, yPos, xPos+WIDTH+20, yPos+WIDTH+20 ,frameX,frameY,frameX+WIDTH , frameY+WIDTH,observer);
  }
}