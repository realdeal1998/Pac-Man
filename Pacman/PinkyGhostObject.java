...
public class PinkyGhostObject extends GameObject
{
  BufferedImage pinkyImage = null , powerUpImage = null , powerUpEndingImage = null;
  
  public PinkyGhostObject(int x , int y , int spawnWait)
  {
    try
    {
      pinkyImage = ImageIO.read(new File("pinkySpriteSheet.png"));
  ...
  public void hasToMakeDecision(int[]dataBuffInt , int w , int pacmanXPos , int pacmanYPos  , Ellipse2D.Double pacmanBounds)
  {
    ...
    if(flag ==0)
    {
      ...
      else if(scatterTimer < 800) makeRandomDecision2(dataBuffInt,w,pacmanXPos,pacmanYPos);
      else 
      {
        if(pacmanXPos==xPos){
          startVertical = true;
  ...
  {
    if(ghostTimer==3) g.drawString(""+eatingScore,xPos,yPos+25);
    else if(!powerUp)g.drawImage(pinkyImage,xPos, yPos, xPos+WIDTH, yPos+WIDTH ,frameX,frameY,frameX+WIDTH , frameY+WIDTH,observer);
  ...
  {
     g.drawImage(pinkyImage,xPos, yPos, xPos+WIDTH+20, yPos+WIDTH+20 ,frameX,frameY,frameX+WIDTH , frameY+WIDTH,observer);
  }
}